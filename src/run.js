...
function run(dir1, dir2, kYype, ip) {
  ...
  if (dir1 && dir2 && kYype) {
    ...
    const nowRoadhogrc = roadhogrc.replace(/dev/g, kYype).replace(/test/g, kYype).replace(/tst/g, kYype).replace(/ttt/g, kYype);
    fs.writeFile(dir2 + '/.roadhogrc', nowRoadhogrc, function(err) {
      ...
      fs.writeFile(dir1 + '/.roadhogrc', nowRoadhogrc, function(err) {
        ...
        nodeCmd.get(`${driveLetterDir1}:&cd ${dir1}&npm start`, function(err, data, stderr) {
          console.log(data);
        });
        console.log('running ...' + dir2);
        nodeCmd.get(`${driveLetterDir2}:&cd ${dir2}&npm start`, function(err, data, stderr) {
          console.log(data);
        });
      });
    });
